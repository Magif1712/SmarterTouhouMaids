from urana_process.fittable_mapper.nn.original_cnn.mapping.training.cnn_training_ops_native import _cnnBackwardLayer

# 桥接层（对应 Java CnnTrainingOps）：接收上层对象，提取句柄，调用原生方法。
# bufHp 始终更新权重+刷新 cache（从 bufHp 提取 p/q/l/r/b/idx/w 句柄）。
# bufTc 非 None 时外拷输入层梯度；None 时跳过（传 0）。
#
