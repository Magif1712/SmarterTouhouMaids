from urana_process.fittable_mapper.nn.original_cnn.mapping.inference.cnn_inference_ops_native import _cnnForwardLayer, _cnnRefreshCache

# 桥接层（对应 Java CnnInferenceOps）：接收上层对象，提取句柄，调用原生方法。
# trace 为 None 时纯推理（NoTrace：z 用 y 做工作区，push→pull→activate 覆盖 y）；
# 非 None 时训练前向（StoreTrace：z=trace.z 累加，y=trace.y 写 σ(z)）。
# cnnRefreshCache 由 p 重算 idx0/idx1/w0/w1（非热路径，stream 0 + 同步）。


def cnnForwardLayer(x, hp, stream, _: "->", y, trace):
    traceZ = trace.z.requireHandle() if trace is not None else 0
    traceY = trace.y.requireHandle() if trace is not None else 0
    _cnnForwardLayer(
        x.requireHandle(),
