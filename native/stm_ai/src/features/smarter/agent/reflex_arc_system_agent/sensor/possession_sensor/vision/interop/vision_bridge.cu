    ...
    cudaStream_t stream)
{
    ...
    try {
        ...
        err = launchUnpackKernel(texObj, dstPtr, aiWidth, aiHeight, sampleWidth, sampleHeight, wordsPerPlane, stream);
        ...
        std::cerr << "[Vision] Error in captureScreenViaSnapshotBridge: " << e.what() << std::endl;
