...
{
    if (!dst || !src) {
        throw std::invalid_argument("Received null pointer in negateAndBinarizeBridge.");
    }
    ...
    try
    {
        cudaError_t err = negateAndBinarize(
            dst->data(),
            src->data(),
            src->size(),
            stream
        ...
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("CUDA error in negateAndBinarize: ") + cudaGetErrorString(err));
        }
    }
...
{
    if (!dst || !src) {
        throw std::invalid_argument("Received null pointer in negateAndBinarizeRegionBridge.");
    }
    ...
    try
    {
        cudaError_t err = negateAndBinarizeRegion(
            dst->data(),
            dst_offset,
            src->data(),
            src_offset,
        ...
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("CUDA error in negateAndBinarizeRegion: ") + cudaGetErrorString(err));
        }
    }
    catch (const std::exception& e)
    {
