...
)
{
    ...
    try {
        ...
        // GL_NEAREST 跨步抽样。因此有效解码区域 = snapshotTexture 全域，
        // 输入输出 1:1（inWidth=outWidth、inHeight=outHeight）。
        ...
        int wordsPerPlane = (width * height + 31) / 32;

        err = launchUnpackKernel(texObj, width, height, width, height, wordsPerPlane, stream, /* -> */ dstPtr);
        ...
        std::cerr << "[Vision] Error in bitplaneEncodeBridge: " << e.what() << std::endl;
