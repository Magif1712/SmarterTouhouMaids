...
)
{
    ...
    // 1. 计算 dz1（gradientSignFlip 会覆盖 dz1 的全部内容，无需清零）
    cudaError_t err = gradientSignFlip(dz1, da1,
                         fz_packed,
                         b_packed,
        ...
        for (int k = 0; k < ELEMS_PER_THREAD; ++k)
        {
            ...
            if (idx < n_prev)
            {
                ...
                else
                {
                    ...
                    const int im1 = i - 1;
                    // 必须同时保证 im1 < n_curr，否则 dz[im1] 与 get_bit_val(r_packed, im1)
                    // 都会越界（当 n_prev > n_curr 且 i 接近 n_prev 时 im1 可能 >= n_curr）。
                    if (im1 >= 0 && im1 < n_curr && pi == im1)
                    {
    ...
    cudaStream_t stream)
{
    ...
    cudaError_t err = gradientSignFlip(dz, da1, fz_packed, b_packed, static_cast<size_t>(n_curr), stream);
    if (err != cudaSuccess) {
