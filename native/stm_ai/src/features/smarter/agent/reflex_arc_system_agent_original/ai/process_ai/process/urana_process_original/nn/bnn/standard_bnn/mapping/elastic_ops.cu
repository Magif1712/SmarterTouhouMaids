...
__global__ void reconnectOnInputChangeKernel_original(
...
__global__ void copyWordsKernel_original(
...
cudaError_t reconnectOnInputChange_original(
    uint32_t*       __restrict__ q_data,
    const uint32_t* __restrict__ input_data,
    ...
    cudaStream_t    stream)
{
    ...
    reconnectOnInputChangeKernel_original<<<blocks, threads, 0, stream>>>(
        q_data, input_data, prev_data, num_words);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) return err;

    copyWordsKernel_original<<<blocks, threads, 0, stream>>>(
