...
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.fittable_mapper.original_mapper.BitplaneEncoder;
...
public abstract class AbstractBnnNeuralNetwork implements INeuralNetwork {
    ...
    /**
     * BNN 家族的视觉解码器：位平面（w*h*24 bit）→ BoolVector（BNN 载体）。
     * 复用 original_mapper 包的同款 {@link BitplaneEncoder}——native 符号名包含包路径
     * （JNI 命名规则），新包复制副本会 UnsatisfiedLinkError；解码逻辑载体无关，直接复用。
     */
    @Override
     ...
     * （梯度外拷）。CNN 下两者同体（FloatVector）无矛盾；BNN 下前向=BoolVector、梯度=IntVector 不兼容。
