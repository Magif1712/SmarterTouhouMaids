...
public class UranaProcessFactory implements ProcessFactory, DebugPanelProvider, ParamPanelProvider, PersistableProvider {
    ...
    public IProcessSystem create(CompoundTag config, EntityMaid maid, SaveSlot slot) {
        // === 查 NN_LEGACY registry 取下层 nn factory（自驱组装，slot 透传供 nn load 权重）===
        // 旧版 INeuralNetwork 接口与新版本不兼容，旧版 nn 迁到 NN_LEGACY registry。
        // nnId 缺失/非法时回退 NN_LEGACY 默认 entry（Registry.resolve 内置 fallback）。
        Registry<?> nnRegistry = RegistryManager.INSTANCE.get(LegacyRegistryIds.NN_LEGACY);
        RegistryEntry<?> nnEntry = nnRegistry.resolve(config.getString(LegacyRegistryIds.NN_LEGACY.toString()));
        ...
        // 换 nn 实现（BNN→CNN）时，新 factory 返回自己的 profile，本工厂零改动（真善美第3条）。
        ...
        // === domain 注入 UranaSystem（替代 feelingSize/behaviorSize；profile 已下沉到 nn）===
