...
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process_original.nn.bnn.AbstractBnnNeuralNetwork;
 ...
 *       {@link com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process_original.nn.INeuralNetwork INeuralNetwork}
 *       接口访问，不知道门控重连的存在。</li>
...
public class StandardBnnNeuralNetwork extends AbstractBnnNeuralNetwork {
    ...
    private final BoolVector prevInput_original;
    ...
    public StandardBnnNeuralNetwork(int inputSize, int outputSize) {
        ...
        this.prevInput_original = new BoolVector(inputSize);
    }
     ...
     * <p>
     * prevInput_original 初始为全零，因此第一次 forward 时所有活跃输入位对应的断连通道都会被重连。
     */
    private void reconnectOnInputChange(long stream) {
        ElasticOpsNative._reconnectOnInputChange(
                io_original.getInput().getVector().requireHandle(),
                prevInput_original.requireHandle(),
                networkData_original.getHyperparameters().getQ().requireHandle(),
                stream);
    }
    ...
    public void close() throws Exception {
        if (prevInput_original != null) prevInput_original.close();
        super.close();
    }
}
